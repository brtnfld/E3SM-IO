...
INDIR=${!INDIR}
APIS=(adios pnetcdf hdf5 hdf5_log)
STRATES=(canonical blob log)
OPTIONS=(1100 1110 1111)
FFREQS=(VAR_RECS)
OPS=(VAR_OP)
                    ...
                    do
                        ...
                        do
                            ...
                            do
                                ...
                                if [ "$OP" = "w" ] ; then
                                    echo "srun -n ${NP} -t ${RTL} --cpu_bind=cores  ./${APP} -g ${NG} -k -o ${OUTDIR}/testfile -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p ${CONFIG}"
                                    srun -n ${NP} -t ${RTL} --cpu_bind=cores  ./${APP} -g ${NG} -k -o ${OUTDIR}/testfile -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p ${CONFIG}
                                else
                                    echo "srun -n ${NP} -t ${RTL} --cpu_bind=cores  ./${APP} -g ${NG} -k -i ${OUTDIR}/testfile_rd -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p ${CONFIG}"
                                    srun -n ${NP} -t ${RTL} --cpu_bind=cores  ./${APP} -g ${NG} -k -i ${OUTDIR}/testfile_rd -a ${API} -f ${HX} -r ${NREC} -x ${STRATE} -p ${CONFIG}
                                fi
